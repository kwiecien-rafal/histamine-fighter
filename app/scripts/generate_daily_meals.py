...
Run it (database up, migrations applied, a tool-calling model configured):
    ...
    uv run python -m app.scripts.generate_daily_meals --horizon 7
    uv run python -m app.scripts.generate_daily_meals --date 2026-06-20
    uv run python -m app.scripts.generate_daily_meals --meal-type dinner
"""
...
async def generate(targets: Sequence[date], meal_types: Sequence[MealType]) -> None:
    ...
    async with SessionLocal() as session:
        ...
        cutoff = daily_service.earliest_readable_date(datetime.now(UTC).date())
        pruned = await daily_service.prune_before(cutoff)
