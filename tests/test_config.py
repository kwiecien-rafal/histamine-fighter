...
from app.config import DEV_JWT_SIGNING_KEY, Settings
...
def _clear_secret_env(monkeypatch: pytest.MonkeyPatch) -> None:
    for var in ("JWT_SIGNING_KEY", "DEBUG", "PUBLIC_DEPLOYMENT"):
...
def test_defaults_use_the_placeholder_in_dev() -> None:
    ...
    assert settings.jwt_signing_key == DEV_JWT_SIGNING_KEY


def test_blank_secret_falls_back_to_the_placeholder() -> None:
    assert Settings(_env_file=None, jwt_signing_key="   ").jwt_signing_key == DEV_JWT_SIGNING_KEY
...
def test_production_rejects_a_short_secret() -> None:
    with pytest.raises(ValidationError):
        Settings(_env_file=None, public_deployment=True, jwt_signing_key="too-short")


def test_production_rejects_a_blank_secret() -> None:
    # Blank coerces to the placeholder, which is then rejected in production.
    with pytest.raises(ValidationError):
        Settings(_env_file=None, debug=False, jwt_signing_key="")


def test_strong_secret_boots_in_production() -> None:
    assert Settings(_env_file=None, public_deployment=True, jwt_signing_key=_STRONG_SECRET)
    assert Settings(_env_file=None, debug=False, jwt_signing_key=_STRONG_SECRET)
...
def test_cookie_is_secure_only_on_a_public_deployment() -> None:
    ...
    assert Settings(
        _env_file=None, public_deployment=True, jwt_signing_key=_STRONG_SECRET
    ).cookie_secure
    assert (
        Settings(_env_file=None, debug=False, jwt_signing_key=_STRONG_SECRET).cookie_secure is False
    )
...
def test_app_base_url_drops_the_trailing_slash() -> None:
    ...
    settings = Settings(_env_file=None, app_base_url="https://histaminefighter.com/")
    assert settings.app_base_url == "https://histaminefighter.com"


def test_app_base_url_requires_an_http_scheme() -> None:
    with pytest.raises(ValidationError):
        Settings(_env_file=None, app_base_url="histaminefighter.com")
