...
async def test_lookup_reports_compatibility_for_a_known_ingredient(
...
async def test_lookup_falls_back_to_the_category_on_an_ingredient_miss(
    session: AsyncSession,
) -> None:
...
async def test_lookup_ignores_the_category_when_the_ingredient_is_indexed(
    session: AsyncSession,
) -> None:
...
async def test_lookup_contains_a_database_error_as_unknown(
    ...
    # comes back as "unknown" so the assessment can continue cautiously.
    service = IngredientService(session)
